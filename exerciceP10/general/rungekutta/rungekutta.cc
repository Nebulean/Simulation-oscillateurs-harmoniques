...
void RungeKutta::evolue(Oscillateur& osc, double dt, double t)
{
  ...
  k2 = osc.Q() + dt/2 * l1; // k2 = Q(n-1) + dt/2 * l1
  l2 = osc.f(t + dt/2); // // f(T(n-1) + dt/2, P(n-1) + dt/2 * k1, Q(n-1) + dt/2 * l1)
  k3 = q + dt/2 * l2; // Q(n-1) + dt/2 * l2
  l3 = osc.f(t + dt/2); // f(T(n-1) + dt/2, P(n-1) + dt/2 * k2, Q(n-1) + dt/2 * l2)
  k4 = q + dt*l3; // Q(n-1) + dt * l3
  l4 = osc.f(t + dt); // f(T(n), P(n-1) + dt * k3, Q(n-1) + dt * l3)
