...
### Travail accompli
Nous avons réalisé l'entiereté de P8 et P9. Ceci sont désormais fonctionnel, car nous avons corrigé la plupart des problèmes d'implémentation de la semaine précédente.
### Problèmes rencontrés
...
Ensuite s'est posé la question de savoir si il fallait que chaque oscillateurs (pendule, ressort, ...) héritaient de Oscillateur **et** de Dessinable, héritage multiple donc, ou alors s'il héritaient de Dessinable au travers d'Oscillateur par transitivité. C'est ce deuxième choix que nous avons retenu, malgré qu'il nous ait demandé plus de compréhension des conceptes abordés.
...
Après de *très longues* reflexions, nous avons finalement implémentés une méthode *affiche()* dans Système, qui appelle *dessine()* de chaque oscillateur. Ainsi, nous pouvons dessiner tous les éléments de système, facilement.
Remarquons également que cette implémentation devrait être suffisement générale pour fonctionner avec OpenGL plus tard. 
